...
def load_data():
    ...
    for col in ["No Nota", "Tanggal Masuk", "Estimasi Selesai",
                "Nama Pelanggan", "No HP", "Barang",
                "Kerusakan", "Kelengkapan", "Status", "Harga Jasa"]:
...
def show():
    ...
    with st.form("form_service"):
        ...
        submitted = st.form_submit_button("💾 Simpan & Kirim Nota Servis")
...
Terima Kasih 🙏"""
        ...
        st.markdown(f"[📲 Klik di sini untuk Kirim Nota via WhatsApp]({link})", unsafe_allow_html=True)
    ...
    for i, row in df.iterrows():
        with st.expander(f"{row['Nama Pelanggan']} - {row['Barang']} ({row['Status']})"):
            ...
            with col2:
                if st.button(f"💬 Kirim Nota WA #{i}", key=f"wa_{i}"):
                    msg = f"""NOTA ELEKTRONIK
